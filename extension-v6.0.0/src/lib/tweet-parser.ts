...
export class TwitterParser extends BaseParser implements ContentParser {
  getContent(): null | PostData {
    try {
      ...
      const quotedTweet = extractQuotedTweet(
        doc.querySelector('[role="link"][tabindex="0"]'),
      );
  ...
  async setText(text: string): Promise<void> {
    try {
      const editor = document.querySelector(
        '[data-testid="tweetTextarea_0RichTextInputContainer"] .public-DraftEditor-content',
      ) as HTMLElement;
      if (!editor) {
...
function extractImages(container: Element | null): string[] {
  ...
  return Array.from(
    container.querySelectorAll('[data-testid="tweetPhoto"] img'),
  )
...
function extractUserInfo(container: Element | null): UserInfo {
  if (!container) return { handle: "", username: "" };

  const spans = Array.from(container.querySelectorAll("span"));
  const handleElement = spans.find((span) =>
...
function extractVideo(container: Element | null): undefined | VideoInfo {
  ...
  const videoUrl =
    videoSource?.getAttribute("src") || videoElement.getAttribute("src");
