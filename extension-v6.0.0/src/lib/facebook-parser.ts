...
export class FacebookParser extends BaseParser implements ContentParser {
  ...
  private readonly SELECTORS = {
    ...
    IMAGE: ["img.x1ey2m1c", "img.x1997x5c", ".x78zum5 img"],
    POST_CONTAINER: [".x1yztbdb", '[role="article"]', ".xh8yej3.x1n2onr6"],
    TEXT: [
    ...
    USERNAME: [".x193iq5w.xeuugli strong span", ".x193iq5w.xeuugli a span"],
    VIDEO: {
      CONTAINER:
        '.x1lliihq video, [data-video-id], [data-ad-preview="message"] video',
      PLAYER: "video",
  ...
  async setText(text: string, target?: Element): Promise<void> {
    try {
      ...
      const editor = postContainer.querySelector(
  ...
  private extractImages(container: Element): string[] {
    return this.extractElements<HTMLImageElement>(
      container,
  ...
  private extractText(container: Element): string {
    ...
    if (backgroundPost) {
      ...
      if (text && text !== "Facebook") return text;
    }

    // Fall back to regular text extraction
    const textElements = this.extractElements(
      container,
      this.SELECTORS.TEXT,
      true,
    );
  ...
  private extractVideo(container: Element) {
    ...
    const videoElement = container.querySelector(
      this.SELECTORS.VIDEO.CONTAINER,
    );
