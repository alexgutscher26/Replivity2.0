...
export default defineContentScript({
  ...
  async main(ctx) {
    ...
    let activeStatusUi: Awaited<ReturnType<typeof createShadowRootUi>> | null =
      null;
    ...
    const createStatusUi = async () => {
      ...
      activeStatusUi = await createShadowRootUi(ctx, {
        ...
        onMount: (container) => {
          ...
          root.render(<Status source="facebook" />);
          return { root };
        },
        onRemove: (elements) => {
    ...
    const mountForForms = async () => {
      const forms = document.querySelectorAll(
        'form.x1ed109x.x1n2onr6.xmjcpbm[role="presentation"] > div.xh8yej3:first-of-type',
      );
        ...
        if (!mountedUIs.has(form)) {
          ...
          const ui = await createShadowRootUi(ctx, {
            ...
            onMount: (container) => {
              ...
              const root = ReactDOM.createRoot(container);
              root.render(<Tone source="facebook" />);
