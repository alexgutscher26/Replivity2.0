...
export const replierAccount = pgTable(
  "replier_account",
  {
    id: text("id").primaryKey().notNull(),
...
export const replierSession = pgTable(
  "replier_session",
  {
    id: text("id").primaryKey().notNull(),
    expiresAt: timestamp("expires_at", { mode: "string" }).notNull(),
  ...
  (table) => {
    return {
      ...
      replierSessionTokenUnique: unique("replier_session_token_unique").on(
        table.token,
      ),
    };
  },
...
export const replierBilling = pgTable(
  "replier_billing",
  {
    id: uuid("id").defaultRandom().primaryKey().notNull(),
    userId: text("user_id").notNull(),
  ...
  (table) => {
    return {
      activeSubIdx: index("active_sub_idx").using(
        "btree",
        table.userId.asc().nullsLast(),
        table.productId.asc().nullsLast(),
        table.status.asc().nullsLast(),
      ),
      productIdIdx: index("product_id_idx").using(
        "btree",
        table.productId.asc().nullsLast(),
      ),
      statusIdx: index("status_idx").using(
        "btree",
        table.status.asc().nullsLast(),
      ),
      userIdIdx: index("user_id_idx").using(
        "btree",
        table.userId.asc().nullsLast(),
      ),
...
export const replierGeneration = pgTable(
  "replier_generation",
  {
    id: uuid("id").defaultRandom().primaryKey().notNull(),
    userId: text("user_id").notNull(),
  ...
  (table) => {
    return {
      generationProductIdIdx: index("generation_product_id_idx").using(
        "btree",
        table.productId.asc().nullsLast(),
      ),
      generationSourceIdx: index("generation_source_idx").using(
        "btree",
        table.source.asc().nullsLast(),
      ),
      generationUserIdIdx: index("generation_user_id_idx").using(
        "btree",
        table.userId.asc().nullsLast(),
      ),
      generationUserProductIdx: index("generation_user_product_idx").using(
        "btree",
        table.userId.asc().nullsLast(),
        table.productId.asc().nullsLast(),
      ),
...
export const replierPost = pgTable(
  "replier_post",
  {
    id: integer("id")
      .primaryKey()
      .generatedByDefaultAsIdentity({
        name: "replier_post_id_seq",
        startWith: 1,
        increment: 1,
        minValue: 1,
        maxValue: 2147483647,
  ...
  (table) => {
    return {
      createdByIdx: index("created_by_idx").using(
        "btree",
...
export const replierUsage = pgTable(
  "replier_usage",
  {
    id: uuid("id").defaultRandom().primaryKey().notNull(),
    userId: text("user_id").notNull(),
  ...
  (table) => {
    return {
      usageProductIdIdx: index("usage_product_id_idx").using(
        "btree",
        table.productId.asc().nullsLast(),
      ),
      usageUserIdIdx: index("usage_user_id_idx").using(
        "btree",
        table.userId.asc().nullsLast(),
      ),
      usageUserProductIdx: index("usage_user_product_idx").using(
        "btree",
        table.userId.asc().nullsLast(),
        table.productId.asc().nullsLast(),
      ),
...
export const replierTwoFactor = pgTable(
  "replier_two_factor",
  {
    id: text("id").primaryKey().notNull(),
...
export const replierUser = pgTable(
  "replier_user",
  {
    id: text("id").primaryKey().notNull(),
    name: text("name").notNull(),
  ...
  (table) => {
    return {
      replierUserEmailUnique: unique("replier_user_email_unique").on(
        table.email,
      ),
    };
  },
...
export const replierSecurityEvent = pgTable(
  "replier_security_event",
  {
    id: text("id").primaryKey().notNull(),
    userId: text("user_id").notNull(),
...
export const replierBlogPost = pgTable(
  "replier_blog_post",
  {
    id: integer("id")
      .primaryKey()
      .generatedByDefaultAsIdentity({
        name: "replier_blog_post_id_seq",
        startWith: 1,
        increment: 1,
        minValue: 1,
        maxValue: 2147483647,
  ...
  (table) => {
    return {
      blogPostCreatedByIdx: index("blog_post_created_by_idx").using(
        "btree",
        table.createdBy.asc().nullsLast(),
      ),
      blogPostPublishedAtIdx: index("blog_post_published_at_idx").using(
        "btree",
        table.publishedAt.asc().nullsLast(),
      ),
      blogPostSlugIdx: index("blog_post_slug_idx").using(
        "btree",
        table.slug.asc().nullsLast(),
      ),
      blogPostStatusIdx: index("blog_post_status_idx").using(
        "btree",
        table.status.asc().nullsLast(),
      ),
      ...
      replierBlogPostSlugUnique: unique("replier_blog_post_slug_unique").on(
        table.slug,
      ),
    };
  },
...
export const replierBlogCategory = pgTable(
  "replier_blog_category",
  {
    id: integer("id")
      .primaryKey()
      .generatedByDefaultAsIdentity({
        name: "replier_blog_category_id_seq",
        startWith: 1,
        increment: 1,
        minValue: 1,
        maxValue: 2147483647,
  ...
  (table) => {
    return {
      blogCategoryNameIdx: index("blog_category_name_idx").using(
        "btree",
        table.name.asc().nullsLast(),
      ),
      blogCategorySlugIdx: index("blog_category_slug_idx").using(
        "btree",
        table.slug.asc().nullsLast(),
      ),
      replierBlogCategorySlugUnique: unique(
        "replier_blog_category_slug_unique",
      ).on(table.slug),
    };
  },
);

export const replierBlogTag = pgTable(
  "replier_blog_tag",
  {
    id: integer("id")
      .primaryKey()
      .generatedByDefaultAsIdentity({
        name: "replier_blog_tag_id_seq",
        startWith: 1,
        increment: 1,
        minValue: 1,
        maxValue: 2147483647,
  ...
  (table) => {
    return {
      blogTagNameIdx: index("blog_tag_name_idx").using(
        "btree",
        table.name.asc().nullsLast(),
      ),
      blogTagSlugIdx: index("blog_tag_slug_idx").using(
        "btree",
        table.slug.asc().nullsLast(),
      ),
      replierBlogTagSlugUnique: unique("replier_blog_tag_slug_unique").on(
        table.slug,
      ),
    };
  },
...
export const replierBlogComment = pgTable(
  "replier_blog_comment",
  {
    id: integer("id")
      .primaryKey()
      .generatedByDefaultAsIdentity({
        name: "replier_blog_comment_id_seq",
        startWith: 1,
        increment: 1,
        minValue: 1,
        maxValue: 2147483647,
  ...
  (table) => {
    return {
      blogCommentAuthorIdx: index("blog_comment_author_idx").using(
        "btree",
        table.authorId.asc().nullsLast(),
      ),
      blogCommentCreatedAtIdx: index("blog_comment_created_at_idx").using(
        "btree",
        table.createdAt.asc().nullsLast(),
      ),
      blogCommentEmailIdx: index("blog_comment_email_idx").using(
        "btree",
        table.authorEmail.asc().nullsLast(),
      ),
      blogCommentParentIdx: index("blog_comment_parent_idx").using(
        "btree",
        table.parentId.asc().nullsLast(),
      ),
      blogCommentPostIdx: index("blog_comment_post_idx").using(
        "btree",
        table.postId.asc().nullsLast(),
      ),
      blogCommentStatusIdx: index("blog_comment_status_idx").using(
        "btree",
        table.status.asc().nullsLast(),
      ),
...
export const replierBlogPostCategory = pgTable(
  "replier_blog_post_category",
  {
    postId: integer("post_id").notNull(),
  ...
  (table) => {
    return {
      blogPostCategoryCategoryIdx: index(
        "blog_post_category_category_idx",
      ).using("btree", table.categoryId.asc().nullsLast()),
      blogPostCategoryPostIdx: index("blog_post_category_post_idx").using(
        "btree",
        table.postId.asc().nullsLast(),
      ),
      ...
      replierBlogPostCategoryPostIdCategoryIdPk: primaryKey({
        columns: [table.postId, table.categoryId],
        name: "replier_blog_post_category_post_id_category_id_pk",
      }),
    };
  },
);

export const replierBlogPostTag = pgTable(
  "replier_blog_post_tag",
  {
    postId: integer("post_id").notNull(),
  ...
  (table) => {
    return {
      blogPostTagPostIdx: index("blog_post_tag_post_idx").using(
        "btree",
        table.postId.asc().nullsLast(),
      ),
      blogPostTagTagIdx: index("blog_post_tag_tag_idx").using(
        "btree",
      ...
      replierBlogPostTagPostIdTagIdPk: primaryKey({
        columns: [table.postId, table.tagId],
        name: "replier_blog_post_tag_post_id_tag_id_pk",
      }),
    };
  },
);

export const replierBlogCommentLike = pgTable(
  "replier_blog_comment_like",
  {
  ...
  (table) => {
    return {
      blogCommentLikeCommentIdx: index("blog_comment_like_comment_idx").using(
        "btree",
        table.commentId.asc().nullsLast(),
      ),
      blogCommentLikeUserIdx: index("blog_comment_like_user_idx").using(
        "btree",
        table.userId.asc().nullsLast(),
      ),
      ...
      replierBlogCommentLikeCommentIdUserIdPk: primaryKey({
        columns: [table.commentId, table.userId],
        name: "replier_blog_comment_like_comment_id_user_id_pk",
      }),
    };
  },
);
