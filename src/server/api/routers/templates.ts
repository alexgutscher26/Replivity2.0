...
export const templatesRouter = createTRPCRouter({
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const userSubscription = await ctx.db.query.billing.findFirst({
        where: (billing, { eq, and }) =>
          and(eq(billing.userId, userId), eq(billing.status, "active")),
        with: {
          product: true,
        },
      });
      ...
      const currentUsage = await ctx.db.query.usage.findFirst({
        where: (usage, { eq, and }) => and(eq(usage.userId, userId)),
      });

      const usageLimit = (userSubscription.product as { name?: string })?.name
        ?.toLowerCase()
        .includes("pro")
        ? 100
        : 20; // Example limits
  ...
  getAll: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const templates = await ctx.db.query.hashtagTemplates.findMany({
        ...
        orderBy: (hashtagTemplates, { desc }) => [
          desc(hashtagTemplates.createdAt),
        ],
        limit: input.limit,
        offset: input.offset,
      });
      ...
      const totalCountResult = await ctx.db
        ...
        .where(
          and(
            ...
            ...(input.category
              ? [eq(hashtagTemplates.category, input.category)]
              : []),
          ),
        );
  ...
  duplicate: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const duplicatedTemplate = await ctx.db
        .insert(hashtagTemplates)
        .values({
          id: randomUUID(),
  ...
  getCategories: protectedProcedure.query(async ({ ctx }) => {
    ...
    const categories = await ctx.db
      .selectDistinct({ category: hashtagTemplates.category })
      .from(hashtagTemplates)
      .where(
        and(
          eq(hashtagTemplates.userId, userId),
  ...
  getUsageStats: protectedProcedure.query(async ({ ctx }) => {
    ...
    const usageLimit = (
      userSubscription?.product as { name?: string } | undefined
    )?.name
      ?.toLowerCase()
      .includes("pro")
      ? 100
