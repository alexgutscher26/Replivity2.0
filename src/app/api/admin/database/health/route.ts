...
export async function GET() {
  try {
    ...
    const response = {
      ...
      recommendations: generateRecommendations(healthStatus, performanceSummary, slowQueries),
    };
...
function generateRecommendations(
  ...
  if (healthStatus.latency > 100) {
    recommendations.push("High database latency detected. Consider optimizing network connection or moving closer to database.");
  }

  if (healthStatus.connections.active / healthStatus.connections.total > 0.8) {
    recommendations.push("High connection pool usage. Consider increasing max connections or optimizing query patterns.");
  }

  // Query performance recommendations
  if (slowQueries.length > 5) {
    recommendations.push(`${slowQueries.length} slow queries detected. Review and optimize these queries or add missing indexes.`);
  }

  if (performanceSummary.cacheSize > 1000) {
    recommendations.push("Large cache size detected. Consider implementing cache expiration policies.");
  }

  if (performanceSummary.totalQueries > 100) {
    const slowQueryRatio = performanceSummary.slowQueries / performanceSummary.totalQueries;
    if (slowQueryRatio > 0.1) {
      recommendations.push(`${Math.round(slowQueryRatio * 100)}% of queries are slow. Consider query optimization and indexing.`);
    }
  }
  ...
  if (frequentSlowQueries.length > 0) {
    recommendations.push(`Frequently executed slow queries detected: ${frequentSlowQueries.map(q => q.queryId).join(', ')}`);
  }

  // Cache recommendations
  if (performanceSummary.cacheSize === 0) {
    recommendations.push("No cached queries detected. Consider implementing caching for frequently accessed data.");
  }
