...
export default function TwoFactorPage() {
  ...
  // Memoized values
  const isBackupMethod = useMemo(() => state.method === 'backup', [state.method]);
        ...
        <Card className="w-full">
          <CardHeader className="text-center space-y-4">
            <div className="mx-auto w-12 h-12 bg-muted rounded-full flex items-center justify-center">
              {isBackupMethod ? (
                <Key className="w-6 h-6 text-muted-foreground" aria-hidden="true" />
              ) : (
          ...
          <CardContent>
            ...
            <div className="mt-6 text-center space-y-3">
              ...
              >
                {isBackupMethod ? "Use authenticator app instead" : "Use backup code instead"}
              </Button>
              <p id="method-toggle-description" className="sr-only">
                Switch between authenticator app and backup code verification methods
              </p>
