...
export function TweetGeneratorForm() {
  ...
  const generateTweet = api.generations.generate.useMutation({
    ...
    onError: (error) => {
      setIsGenerating(false);
      toast.error(
        error.message ?? "Failed to generate tweet. Please try again.",
      );
    },
  });
  ...
  const onSubmit = async (data: TweetGeneratorFormValues) => {
    ...
    if (accountSettings) {
      ...
      if (accountSettings.brandPersonality) {
        brandParts.push(
          `Brand Personality: ${accountSettings.brandPersonality}`,
        );
      }
      ...
      if (accountSettings.brandKeywords) {
        brandParts.push(
          `Include these keywords naturally: ${accountSettings.brandKeywords}`,
        );
      }

      if (accountSettings.avoidKeywords) {
        brandParts.push(
          `Avoid these keywords: ${accountSettings.avoidKeywords}`,
        );
      }
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Generate authentic, human-like tweets that pass AI detection.
            Customize with optional emojis and hashtags.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Describe the main topic or theme for your tweet. Be
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Provide additional context, personal experiences, or
                      specific details to make the tweet more authentic.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Choose how many different tweet variations you'd like to
                      generate.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
          ...
          {generatedTweets.length > 0 ? (
            <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {generatedTweets.map((_, index) => (
                    <Button
                      key={index}
                      variant={
                        selectedTweetIndex === index ? "default" : "outline"
                      }
                      size="sm"
              ...
              <div className="bg-muted/50 rounded-lg border p-4">
                ...
                <div className="border-border/50 text-muted-foreground mt-3 border-t pt-3 text-xs">
                  Character count:{" "}
                  {generatedTweets[selectedTweetIndex]?.length ?? 0}/280
                </div>
              </div>
            ...
            <div className="text-muted-foreground py-12 text-center">
              <Twitter className="mx-auto mb-4 h-12 w-12 opacity-50" />
              <p className="mb-2 text-lg font-medium">No tweet generated yet</p>
              <p className="text-sm">
                Fill out the form and click "Generate Tweet" to create your
                human-like tweet.
              </p>
            </div>
          )}
