...
export default function HashtagCompetitionAnalysis() {
  ...
  // API calls
  const { data: competitionData = [], refetch } =
    api.hashtags.getHashtagCompetitionAnalysis.useQuery({
  ...
  const analyzeMutation = api.hashtags.analyzeHashtagCompetition.useMutation({
    ...
    onError: (error: unknown) => {
      const errorMessage =
        error instanceof Error ? error.message : "Failed to analyze hashtag";
          ...
          <Card key={index} className="relative">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold">
                    {formatNumber(data.recentPosts ?? 0)}
                  </div>
                  <div className="text-muted-foreground text-sm">
                    Recent Posts
                  </div>
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                <div className="space-y-2">
                  ...
                  <Progress
                    value={parseFloat(data.avgEngagement ?? "0")}
                    className="h-2"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Progress
                    value={parseFloat(data.competitionScore ?? "0")}
                    className="h-2"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Progress
                    value={parseFloat(data.opportunity ?? "0")}
                    className="h-2"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`text-sm font-medium ${parseFloat(data.growthRate ?? "0") >= 0 ? "text-green-600" : "text-red-600"}`}
                      >
                        {parseFloat(data.growthRate ?? "0") > 0 ? "+" : ""}
                        {data.growthRate}%
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-medium">
                        {data.peakEngagementDay ?? "Unknown"}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(Array.isArray(data.bestPostingTimes)
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(Array.isArray(data.topContentTypes)
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(Array.isArray(data.relatedHashtags)
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {(Array.isArray(data.topRegions) ? data.topRegions : []).map(
              ...
              <div className="bg-muted/50 rounded-lg p-4">
                ...
                <ul className="text-muted-foreground space-y-1 text-sm">
                  {data.difficulty === "easy" && (
                    <li>
                      • Low competition - Great opportunity for visibility
                    </li>
                  )}
                  {data.difficulty === "medium" && (
                    <li>
                      • Moderate competition - Good balance of reach and
                      competition
                    </li>
                  )}
                  {data.difficulty === "hard" && (
                    <li>
                      • High competition - Requires quality content to stand out
                    </li>
                  )}
                  {data.difficulty === "very_hard" && (
                    <li>
                      • Very high competition - Consider niche alternatives
                    </li>
                  )}
                  ...
                  {parseFloat(data.avgEngagement ?? "0") > 5 && (
                    <li>
                      • High engagement rate - Quality audience interaction
                    </li>
                  )}
                  <li>
                    • Best posting time: {data.peakEngagementDay ?? "Unknown"}{" "}
                    at{" "}
                    {Array.isArray(data.bestPostingTimes) &&
                    data.bestPostingTimes.length > 0
                      ? data.bestPostingTimes[0]
                      : "N/A"}
                    :00
                  </li>
                </ul>
