...
export function mapSocialAuthError(error: Error | string): SocialAuthError {
  ...
  if (
    ...
    errorString.includes("clientsecret")
  ) {
    return {
  ...
  if (errorString.includes("oauth") || errorString.includes("provider")) {
    return {
      ...
      message:
        "There was an issue with the social provider configuration. Please check your settings.",
      code: "PROVIDER_ERROR",
    };
  }
...
export function validateProviderCredentials(
  providers: string[],
  credentials: Record<string, { clientId?: string; clientSecret?: string }>,
): string[] {
...
export function validateAuthSecret(secret: string): {
  ...
  if (/^[a-zA-Z]+$/.test(secret) || /^[0-9]+$/.test(secret)) {
    return {
      isValid: false,
      message:
        "Auth secret should contain a mix of letters, numbers, and special characters.",
    };
  }
 ...
 */
export function validateTrustedOrigins(origins: string[]): {
...
export type SocialAuthErrorCode =
  (typeof SOCIAL_AUTH_ERROR_CODES)[keyof typeof SOCIAL_AUTH_ERROR_CODES];
